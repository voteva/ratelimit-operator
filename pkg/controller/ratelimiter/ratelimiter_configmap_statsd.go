...
func (r *ReconcileRateLimiter) reconcileConfigMapStatsd(ctx context.Context, instance *v1.RateLimiter) (reconcile.Result, error) {
	...
	cmName := instance.Name + "-statsd-exporter"
	foundConfigMap := &corev1.ConfigMap{}

	err := r.Client.Get(ctx, types.NamespacedName{Name: cmName, Namespace: instance.Namespace}, foundConfigMap)
	if err != nil {
		if errors.IsNotFound(err) {
			configMapFromInstance := buildConfigMapStatsd(cmName, instance)
...
func buildConfigMapStatsd(cmName string, instance *v1.RateLimiter) *corev1.ConfigMap {
